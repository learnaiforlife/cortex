...
1. Run the linter with `npm run lint` and capture all output.
...
3. Run the formatter with `npx prettier --check .` in check mode to identify files that need formatting.
4. Report a summary: total errors, total warnings, total files needing formatting.
5. If the user wants to auto-fix, run `npx eslint --fix . && npx prettier --write .` and show the resulting diff before committing changes.
